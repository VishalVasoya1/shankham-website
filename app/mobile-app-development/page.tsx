...
export default function MobileAppDevelopment() {
  ...
  const apps = projectsByCategory["App Development"].map((app) => ({
    ...
    longDescription: (() => {
      switch (app.title) {
        case "Upifyme":
          return "Upifyme is an intelligent cross-platform mobile app for Android and iOS that helps students discover the best-fit colleges and universities through personalized insights. It combines quizzes, gameplay, and video-based observations to generate detailed reports tailored to each user’s profile. The app integrates deep linking with Branch.io, real-time socket-based communication, and a custom chatbot for an interactive experience. Built using Flutter and Unity, and powered by Firebase for analytics, crash reporting, and push notifications, Upifyme exemplifies my ability to build data-driven, engaging, and user-centric educational applications.";
        case "Ace Jackpot":
          return "Ace Jackpot is an immersive casino-themed mobile game developed for Android and iOS, blending strategy and entertainment to deliver a premium gaming experience. Players can compete in tournaments, climb leaderboards, and earn daily rewards while managing their in-game wallet securely. Built using Flutter and Unity, the app ensures smooth performance across platforms, integrating Stripe for payments and social logins for seamless accessibility. This project highlights my expertise in creating engaging, multi-platform games that combine secure transactions, competitive play, and rewarding user interactions.";
        case "V-Translate":
          return "V-Translate is a feature-rich translation app built for Android and iOS that enables users to communicate effortlessly across languages. It supports speech-to-text and text-to-speech using Google APIs, simultaneous multi-language translation, and even works offline for on-the-go use. The app allows users to share translations directly on Twitter or WhatsApp and offers AI-assisted extended translations for deeper accuracy. With secure Firebase storage and in-app purchases via RevenueCat, V-Translate delivers a smooth and accessible experience for users worldwide, showcasing my ability to build intelligent, globally connected mobile applications.";
        default:
          return "";
      }
    })(),
  ...
  React.useEffect(() => {
    ...
    api.on("select", () => {
      ...
      for (let i = 0; i < apps.length; i++) {
        const appSlideCount = apps[i]?.images?.length;
        if (selectedIndex < totalSlides + appSlideCount!) {
          appIndex = i;
          break;
        }
        totalSlides += appSlideCount!;
      }
  ...
  const allSlides = apps.flatMap((app) => {
    return app?.images?.map((image) => ({
      ...app,
      currentMedia: image,
    }));
  });
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <div className="text-center mb-20">
            ...
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Explore our collection of powerful mobile applications
            </p>
          </div>
          ...
          <div className="relative max-w-6xl mx-auto">
            {allSlides.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center gap-4 sm:gap-6 flex-wrap">
                      <div className="space-y-3 sm:space-y-4 flex-1 min-w-[240px] sm:min-w-[280px]">
                        ...
                        {currentApp.longDescription && (
                          <p className="text-muted-foreground text-sm sm:text-base lg:text-lg leading-relaxed max-w-[90%] mx-auto px-3 sm:px-6 mt-2">
                            {currentApp.longDescription}
                          </p>
                        )}
                        ...
                        <div className="flex flex-wrap gap-2 justify-center mt-4">
                          {currentApp.tags.map((tag, idx) => (
                            <span
                              key={idx}
                              className="px-3 py-1 bg-foreground/5 text-foreground/80 text-xs font-medium rounded-lg border border-foreground/10"
                            >
                              {tag}
                            </span>
                          ))}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        for (let i = 0; i < index; i++) {
                          startIndex += apps[i]?.images?.length!;
                        }
                    ...
                    >
                      {(app.images?.length ?? 0) > 0 ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center">
                          <Smartphone className="w-6 h-6 sm:w-8 sm:h-8 text-foreground/30" />
                        </div>
                      )}
              ...
              <div className="text-center py-20 px-6 bg-muted/30 rounded-3xl border-2 border-dashed">
                <Smartphone className="w-20 h-20 text-muted-foreground/30 mx-auto mb-6" />
                <h3 className="text-2xl font-bold text-foreground mb-3">
                  App Screenshots Coming Soon
                </h3>
                <p className="text-muted-foreground text-lg">
                  Add screenshots to showcase your apps
                </p>
              </div>
            )}
          </div>
      ...
      <section className="border-b">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <div className="text-center mb-20">
            ...
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Everything you need to create a successful app from concept to
              launch
            </p>
          </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-muted via-muted/80 to-muted/60 p-12 lg:p-20 text-center border-2">
          <div className="relative z-10 space-y-8">
            ...
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Let's bring your app vision to life and create an experience that
              users will love. Get a free consultation and quote today.
            </p>
